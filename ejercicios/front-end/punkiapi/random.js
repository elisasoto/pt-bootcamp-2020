const getRandomBeer = async () => {
    ...
    const beersURl = `https://api.punkapi.com/v2/beers/random`
...
const start = async () => {
    xtdBeer = await getRandomBeer()
    const [oneBeer] = await getRandomBeer()
        ...
        getGeometry()
            .then(({ coordinates }) => {
                ...
                function onMapClick(e) {
                    popup
                        .setLatLng(e.latlng)
                        .setContent("Brewed for first time on => " + `${xtdBeer[0].firstBrewed}`)
                        .openOn(mymap);
                }
