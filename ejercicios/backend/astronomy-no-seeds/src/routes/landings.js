...
//codigo para parsear a numeros
// db.landings.find().forEach( function (x) {
//   x.id = parseInt(x.id);
//   db.landings.save(x);
//   });
...
route.get("/", async (req, res, next) => {
  ...
  try {
    ...
    const result = await LandingModel.find(
      { $expr: { $gt: [{ $toDouble: "$mass" }, Number(minimum_mass)] } },
      { name: 1, mass: 1, _id: 0 }
    );
...
route.get("/date", async (req, res, next) => {
  try {
    ...
    const result = await LandingModel.aggregate([
      {
        $addFields: {
          justYear: {
            $year: {
...
route.get("/:name", async (req, res, next) => {
  try {
    let result = await LandingModel.find(
