...
// url http://localhost:3000/todos?page=1&limit=3
...
router.get('/', (req, res, next) => {
  try {
    ...
    const filteredToDo = toDoJSON.filter((element) => {
    ...
    res.status(200).json({
      ...
      data: todosList,// dani si cambias aqui a filtered todo entonces podemos filtrar por category. Como hacemos para combinar todo? un if? una funcion? por otro lado como hacemos para traer todas las categorias si no buscamos por id? aqui el find no nos vale o si?
...
router.delete('/:id', (req, res, next) => {
  try {
    const toDoList = fs.readFileSync(toDosDB)
    const toDoJSON = JSON.parse(toDoList)
    const filteringByID = toDoJSON.filter(
